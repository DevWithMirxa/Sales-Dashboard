  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Type",
        accessorFn: (row) =>
          `${row.businessType || ""}${row.businessType === "Farm" && row.farmType ? ` - ${row.farmType}` : ""}`,
      },
      {
        id: "contactPerson",
        header: "Contact Person",
        accessorFn: (row) =>
          getPrimaryContact(row)?.name || row.personName || "-",
      },
      {
        id: "mobile",
        header: "Mobile",
        accessorFn: (row) =>
          getPrimaryContact(row)?.mobile || row.mobile || "-",
      },
      {
        id: "cityRegion",
        header: "City/Region",
          ...
          >
            ...
            {row.original.contacts && row.original.contacts.length > 1 && (
              <div className="ml-2 text-sm text-gray-500">
                {row.original.contacts.length} contacts
              </div>
            )}
          </div>
        ),
              ...
              {customer.contacts && customer.contacts.length > 0 ? (
                <div className="space-y-3">
                  {customer.contacts.map((contact, index) => (
                    <div
                      key={index}
                      className="text-sm text-gray-600 border border-gray-200 rounded-md p-3 bg-white"
                    >
